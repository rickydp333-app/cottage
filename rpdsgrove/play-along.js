(function(){
  ...
  dialog.innerHTML=`<button class="close pa-close" id="paClose" aria-label="Close Play Along">×</button>
    ...
    <div class="pa-controls"><button id="paExport" hidden>Save chord chart</button><button id="paImport">Load chord chart</button><input id="paChartFile" type="file" accept=".json,application/json" hidden></div><div class="pa-timeline" id="paTimeline" aria-label="Chord timeline"></div></div></div>
  ...
  function persist(){if(mode!=='library'||!source)return;try{const k=key(source);let recent=JSON.parse(localStorage.getItem('grove:chord-cache-keys')||'[]');if(!Array.isArray(recent))recent=[];recent=[k,...recent.filter(x=>typeof x==='string'&&x!==k)];for(const old of recent.slice(12))if(old.startsWith('grove:chords:v2:'))localStorage.removeItem(old);localStorage.setItem(k,JSON.stringify(segments));localStorage.setItem('grove:chord-cache-keys',JSON.stringify(recent.slice(0,12)));$('paSaving').textContent='Chord map saved on this device.';}catch{$('paSaving').textContent='Storage unavailable. This chord map is available for this session.';}}
  function timeline(){
