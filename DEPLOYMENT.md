...
Map buttons now open an embedded Google map inside a large kiosk dialog, with Close Map / Back to RDPs Place. No popup is created for the address-query links generated by the guide. The map frame cannot navigate the top-level kiosk page or open new windows. Google Maps must be allowed by device/network policy. Other external links retain secondary-window behavior.
